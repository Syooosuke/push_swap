...
BIN_TARGET="tests/adaptive_test"
...
while [ "$i" -le "$CASES" ]; do
	...
	if ! output="$("$BIN" $numbers 2>&1)"; then
		printf '[NG] random adaptive case %d failed\n' "$i"
		printf '%s\n' "$output"
		exit 1
	fi
	...
	if [ -z "$steps" ] || [ -z "$size" ] || [ -z "$disorder" ]; then
		printf '[NG] random adaptive case %d: could not read test metrics\n' "$i"
		printf '%s\n' "$output"
		exit 1
	fi
	...
	printf '[OK] random adaptive case %d: size=%d disorder=%s steps=%d\n' \
		"$i" "$size" "$disorder" "$steps"
	i=$((i + 1))
done

printf 'All %d random adaptive_sort tests passed. size=%d range=[%d,%d] min=%d max=%d avg=%d\n' \
